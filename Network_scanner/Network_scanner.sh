...
sudo nmap -sn $subnet | grep 'Nmap scan report' | awk '{print $5}' > hosts.txt
...
screenshot()
{
  ...
  eyewitness --web -f url.txt -d ~/Desktop/Finding --no-prompt
  mv hosts.txt url.txt geckodriver.log ~/Desktop/Finding/
